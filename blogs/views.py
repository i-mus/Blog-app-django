...
def post_detail(request, year, month, day, post):
    ...
    similiar_posts = Post.objects.filter(tags__in=post_tags_ids).exclude(id=post.id)
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            results = Post.objects.annotate(
                similiarity = TrigramSimilarity('title', query),
            ).filter(similiarity__gt=0.1).order_by('-similiarity')
